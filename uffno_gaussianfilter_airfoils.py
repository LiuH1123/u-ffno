...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        dec4 = self.dec4(torch.cat([F.interpolate(center, enc4.size()[-2:], mode='bilinear', align_corners=True), enc4],
                                   1))  #(20,128,221,51)
        #dec3 = self.dec3(torch.cat([dec4, enc3], 1))
        dec3 = self.dec3(torch.cat([F.interpolate(dec4, enc3.size()[-2:], mode='bilinear', align_corners=True), enc3],
                                   1))  #(20,64,221,51)
        #dec2 = self.dec2(torch.cat([dec3, enc2], 1))
        dec2 = self.dec2(torch.cat([F.interpolate(dec3, enc2.size()[-2:], mode='bilinear', align_corners=True), enc2],
                                   1))  #(20,32,221,51)
        dec1 = self.dec1(
            torch.cat([F.interpolate(dec2, enc1.size()[-2:], mode='bilinear', align_corners=True), enc1], 1))
...
class DUFNO2d(nn.Module):
    ...
    def forward(self, x):
        ...
        x_c4 = torch.cat((F.interpolate(x_c4, x_c1.size()[-2:], mode='bilinear', align_corners=True), self.w1(x_c1)),
                         dim=1)  #(20,256,85,85)

        x_c5 = self.conv5(x_c4)
        #x_c5 = torch.cat((x_c5, x_c0), dim=1)  # (20,96,85,85)
        x_c5 = torch.cat((F.interpolate(x_c5, x_c0.size()[-2:], mode='bilinear', align_corners=True), self.w2(x_c0)),
                         dim=1)  #(20,256,85,85)
...
################################################################
model = DUFNO2d(width, 2 * modes, modes).cuda()
...
for ep in range(epochs):
    ...
    with open('/mnt/68213B282F65B10C/LiuH/code/u-shape/uffno_filter/result/airfoils/airfoils.txt', 'a') as file:
        print('Epoch-{}, Time-{}, Train-Mse-{},Train-L2-{}, Test-L2-{}, Best-Test-L2-{}'
    ...
    if (ep + 1 == 1) or ((ep + 1) % step_size == 0):
        ...
        ax0=ax[0, 1].pcolormesh(X_small, Y_small, truth_small, shading='gouraud')
        cax0 = fig.add_axes([ax[0,1].get_position().x1 + 0.01, ax[0,1].get_position().y0, 0.01, ax[0,1].get_position().height])
        plt.colorbar(ax0, cax=cax0)

        ax1=ax[1, 1].pcolormesh(X_small, Y_small, pred_small, shading='gouraud')
        cax1 = fig.add_axes([ax[1,1].get_position().x1 + 0.01, ax[1,1].get_position().y0, 0.01, ax[1,1].get_position().height])
        plt.colorbar(ax1, cax=cax1)

        ax2=ax[2, 1].pcolormesh(X_small, Y_small, np.abs(pred_small - truth_small), shading='gouraud')
        cax2 = fig.add_axes([ax[2,1].get_position().x1 + 0.01, ax[2,1].get_position().y0, 0.01, ax[2,1].get_position().height])
        plt.colorbar(ax2, cax=cax2)

        plt.savefig('/mnt/68213B282F65B10C/LiuH/code/u-shape/uffno_filter/result/airfoils/airfoils_colorbar_{}.png'.format(ep + 1))
        plt.show()
