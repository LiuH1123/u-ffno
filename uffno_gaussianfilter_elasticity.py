...
################################################################
model = DUFNO2d(width, modes, modes).cuda()
...
for ep in range(epochs):
    ...
    t2 = default_timer()
    print('Epoch-{}, Time-{}, Train-L2-{}, Test-L2-{}, Best-Test-L2-{}'
          .format(ep + 1, t2 - t1, train_l2, test_l2, best_test_l2))
    with open('/mnt/68213B282F65B10C/LiuH/code/u-shape/uffno_filter/result/elasticity/elasticity.txt', 'a') as file:
        print('Epoch-{}, Time-{},Train-L2-{}, Test-L2-{}, Best-Test-L2-{}'
              .format(ep + 1, t2 - t1, train_l2, test_l2, best_test_l2),file=file)

    if (ep+1==1) or ((ep+1)%step_size==0):
         #torch.save(model, '../model/pipe_' + str(ep))
        truth = y[0].squeeze().detach().cpu().numpy()
        pred = out2[0].squeeze().detach().cpu().numpy()

        fig, ax = plt.subplots(nrows=3, figsize=(16, 16))
        ax0=ax[0].imshow(truth, cmap='RdBu_r',interpolation='none')
        cax0 = fig.add_axes([ax[0].get_position().x1 + 0.01, ax[0].get_position().y0, 0.01, ax[0].get_position().height])
        plt.colorbar(ax0, cax=cax0)
        ax1=ax[1].imshow(pred, cmap='RdBu_r',interpolation='none')
        cax1 = fig.add_axes([ax[1].get_position().x1 + 0.01, ax[1].get_position().y0, 0.01, ax[1].get_position().height])
        plt.colorbar(ax1, cax=cax1)
        ax2=ax[2].imshow(pred-truth, cmap='RdBu_r',interpolation='none')
        cax2 = fig.add_axes([ax[2].get_position().x1 + 0.01, ax[2].get_position().y0, 0.01, ax[2].get_position().height])
        plt.colorbar(ax2, cax=cax2)
        plt.savefig('/mnt/68213B282F65B10C/LiuH/code/u-shape/uffno_filter/result/elasticity/elasticity_{}.png'.format(ep+1))
        plt.show()